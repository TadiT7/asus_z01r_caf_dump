...
check_dumpsys_ok()
{
	setprop asus.savelogmtp.check_dumpsys_ok 0
	sleep 5
	setprop asus.savelogmtp.check_dumpsys_ok 1
	setprop asus.savelogmtp.check_dump $tagstartdump_prop
	for i in $(seq 60); do		
		if [ ".$tagstartdump_prop" == ".0" ]; then
			setprop asus.savelogmtp.check_dumpsys_ok 2
			break
		elif [ ".$tagstartdump_prop" == ".1" ]; then
			setprop asus.savelogmtp.check_dumpsys_ok 3
			sleep 2
			echo "Done"
		elif [ ".$tagstartdump_prop" == "." ]; then
			setprop asus.savelogmtp.check_dumpsys_ok 4
			echo "Done"
			break;
		fi
	done;
	...
	if [ ".$i" == ".1" ]; then
		setprop asus.savelogmtp.check_ok 2
		sleep 100
	fi
}
save_general_log() {
